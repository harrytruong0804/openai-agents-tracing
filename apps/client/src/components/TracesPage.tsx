...
export default function TracesPage() {
  ...
  const getExecutionTime = (trace: Trace): string => {
    if (trace.execution_time === 'N/A' || trace.execution_time === undefined) {
      return 'N/A';
    }
    const seconds = (trace.execution_time / 1000).toFixed(2);
    return `${seconds}s`;
  };
  ...
  return (
    <div className="flex-1 bg-background flex flex-col h-screen overflow-hidden">
      ...
      <div
        className="flex-1 overflow-auto bg-card/50"
        onScroll={(e) => {
            ...
            {loading ? (
              ...
              traces.map((trace) => (
                ...
                >
                  ...
                  <TableCell className="text-muted-foreground max-w-md py-2 px-6">
                    <div className="flex items-center gap-1.5 overflow-hidden">
                      {trace.flow && trace.flow.length > 0 ? (
                        trace.flow.map((agent, index) => (
                          <div
                            key={index}
